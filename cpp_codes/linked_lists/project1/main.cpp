...
static int callback(void *NotUsed, int argc, char **argv, char **azColName)
{
    ...
    {
		cout << left << setw(20) << azColName[i] << setw(20);
	}
	cout << endl;
    ...
    for(int i = 0; i<argc; i++)
    {
       //printf("%s = %s\n", azColName[i], argv[i] ? argv[i] : "NULL");
   		cout << left << setw(20) << argv[i] << setw(20);
    }
    cout << endl;
...
int main(int argc, char* argv[])
{
	...
	Professor i("Kenneth", "Cole", "10.63NB", "bruceallen@aol.com", "718-693-3348");
	...
	{	
		...
		/*ln = profArray[i].getLastName().c_str();
		fn = profArray[i].getFirstName().c_str();
		rn = profArray[i].getRoomNum().c_str();
		em = profArray[i].getEmail().c_str();
		ph = profArray[i].getPhone().c_str();*/
